...
{
    [TestClass]
    public class ExpedientesLaboralesPresentacionUT
    {
        ...
        private void Guardar()
        {
            ...
            this.entidad = new ExpedientesLaborales()
            {
                ...
                Cargo = "Analista",
                SalarioPagado = 2000000m,
                FechaIngreso = new DateTime(2020, 1, 1),
                FechaEgreso = new DateTime(2023, 1, 1),
                Desempeno = "Bueno",
                MotivoSalida = "Finalizacion",
                Persona = this.persona.Id
            };

            // Se guarda por LibPresentacion, que internamente llama Comunicaciones y la API.
            this.entidad = this.iExpedientesLaboralesNegocio!.Guardar(this.entidad!);

            if (this.entidad != null && this.entidad.Id != 0)
                return;

            throw new Exception("Error al Guardar ExpedientesLaborales a traves de la API.");
        }

        private void Consultar()
        {
            // Se consulta por API y se valida que exista el registro guardado.
            var lista = this.iExpedientesLaboralesNegocio!.Consultar();
            var existeEntidad = lista.Any(x => x.Id == this.entidad!.Id);

            if (existeEntidad)
                return;

            throw new Exception("La entidad ExpedientesLaborales guardada no se encontro al Consultar la API.");
        }

        private void Modificar()
        {
            this.entidad!.Cargo = "Coordinador";
            this.entidad.SalarioPagado = 2500000m;

            // Se modifica por API y se valida el dato cambiado.
            var resultadoModificar = this.iExpedientesLaboralesNegocio!.Modificar(this.entidad!);

            if (resultadoModificar != null && resultadoModificar.Cargo == "Coordinador")
                return;

            throw new Exception("Error al Modificar ExpedientesLaborales a traves de la API.");
        }

        private void Borrar()
        {
            // Se borra primero la entidad principal.
            var resultadoBorrar = this.iExpedientesLaboralesNegocio!.Borrar(this.entidad!);

            if (resultadoBorrar == null || resultadoBorrar.Id == 0)
                throw new Exception("Error al Borrar ExpedientesLaborales a traves de la API.");
