...
{
    [TestClass]
    public class BienesMueblesPresentacionUT
    {
        ...
        private void Guardar()
        {
            ...
            {
                Nombre = "UT-BienMueble-" + DateTime.Now.ToString("yyyyMMddHHmmssfff"),
                Descripcion = "Bien mueble de prueba",
                ...
                Tipo = "Equipo",
                Modelo = "Modelo UT",
                UbicacionActual = "Oficina",
                Garantia = "12 meses",
                Estado = "Bueno"
            };

            // Se guarda por LibPresentacion, que internamente llama Comunicaciones y la API.
            this.entidad = this.iBienesMueblesNegocio!.Guardar(this.entidad!);

            if (this.entidad != null && this.entidad.Id != 0)
                return;

            throw new Exception("Error al Guardar BienesMuebles a traves de la API.");
        }

        private void Consultar()
        {
            // Se consulta por API y se valida que exista el registro guardado.
            var lista = this.iBienesMueblesNegocio!.Consultar();
            var existeEntidad = lista.Any(x => x.Id == this.entidad!.Id);

            if (existeEntidad)
                return;

            throw new Exception("La entidad BienesMuebles guardada no se encontro al Consultar la API.");
        }

        private void Modificar()
        {
            this.entidad!.Estado = "Excelente";
            this.entidad.UbicacionActual = "Bodega";

            // Se modifica por API y se valida el dato cambiado.
            var resultadoModificar = this.iBienesMueblesNegocio!.Modificar(this.entidad!);

            if (resultadoModificar != null && resultadoModificar.Estado == "Excelente")
                return;

            throw new Exception("Error al Modificar BienesMuebles a traves de la API.");
        }

        private void Borrar()
        {
            // Se borra primero la entidad principal.
            var resultadoBorrar = this.iBienesMueblesNegocio!.Borrar(this.entidad!);

            if (resultadoBorrar == null || resultadoBorrar.Id == 0)
                throw new Exception("Error al Borrar BienesMuebles a traves de la API.");
