...
{
    [TestClass]
    public class ContratosCodeudoresPresentacionUT
    {
        ...
        private void Guardar()
        {
            ...
            this.contrato = new ContratosNegocio().Guardar(new Contratos()
            {
                ...
                Observaciones = "Contrato para codeudor UT",
            ...
            {
                Nombre = "UT-EstadoCivil-CodeudorContrato-" + DateTime.Now.ToString("yyyyMMddHHmmssfff")
            });

            this.nacionalidadCodeudor = new NacionalidadesNegocio().Guardar(new Nacionalidades()
            {
                Nombre = "UT-Nacionalidad-CodeudorContrato-" + DateTime.Now.ToString("yyyyMMddHHmmssfff")
            });

            this.codeudor = new CodeudoresNegocio().Guardar(new Codeudores()
            {
                Cedula = "1" + DateTime.Now.Ticks.ToString(),
                Nombre = "CodeudorContrato UT",
                Apellido = "Prueba",
                Genero = 'M',
                Correo = "codeudorcontrato" + Guid.NewGuid().ToString("N") + "@correo.com",
                FechaNacimiento = new DateTime(1998, 4, 10),
                FechaRegistro = DateTime.Now,
            ...
            this.entidad = this.iContratosCodeudoresNegocio!.Guardar(this.entidad!);

            if (this.entidad != null && this.entidad.Id != 0)
                return;

            throw new Exception("Error al Guardar ContratosCodeudores a traves de la API.");
        }

        private void Consultar()
        {
            // Se consulta por API y se valida que exista el registro guardado.
            var lista = this.iContratosCodeudoresNegocio!.Consultar();
            var existeEntidad = lista.Any(x => x.Id == this.entidad!.Id);

            if (existeEntidad)
                return;

            throw new Exception("La entidad ContratosCodeudores guardada no se encontro al Consultar la API.");
        }
        ...
        private void Modificar()
        {
            ...
            var resultadoModificar = this.iContratosCodeudoresNegocio!.Modificar(this.entidad!);

            if (resultadoModificar != null && resultadoModificar.VendidaArrendada == "Arrendada")
                return;

            throw new Exception("Error al Modificar ContratosCodeudores a traves de la API.");
        }

        private void Borrar()
        {
            // Se borra primero la entidad principal.
            var resultadoBorrar = this.iContratosCodeudoresNegocio!.Borrar(this.entidad!);

            if (resultadoBorrar == null || resultadoBorrar.Id == 0)
                throw new Exception("Error al Borrar ContratosCodeudores a traves de la API.");
