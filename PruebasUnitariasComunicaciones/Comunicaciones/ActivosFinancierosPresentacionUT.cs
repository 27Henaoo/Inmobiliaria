...
{
    [TestClass]
    public class ActivosFinancierosPresentacionUT
    {
        ...
        private void Guardar()
        {
            ...
            {
                Nombre = "UT-Activo-" + DateTime.Now.ToString("yyyyMMddHHmmssfff"),
            ...
            this.entidad = this.iActivosFinancierosNegocio!.Guardar(this.entidad!);

            if (this.entidad != null && this.entidad.Id != 0)
                return;

            throw new Exception("Error al Guardar ActivosFinancieros a traves de la API.");
        }

        private void Consultar()
        {
            // Se consulta por API y se valida que exista el registro guardado.
            var lista = this.iActivosFinancierosNegocio!.Consultar();
            var existeEntidad = lista.Any(x => x.Id == this.entidad!.Id);

            if (existeEntidad)
                return;

            throw new Exception("La entidad ActivosFinancieros guardada no se encontro al Consultar la API.");
        }

        private void Modificar()
        {
            this.entidad!.Precio = 170000m;
            this.entidad.Descripcion = "Activo modificado";

            // Se modifica por API y se valida el dato cambiado.
            var resultadoModificar = this.iActivosFinancierosNegocio!.Modificar(this.entidad!);

            if (resultadoModificar != null && resultadoModificar.Precio == 170000m)
                return;

            throw new Exception("Error al Modificar ActivosFinancieros a traves de la API.");
        }

        private void Borrar()
        {
            // Se borra primero la entidad principal.
            var resultadoBorrar = this.iActivosFinancierosNegocio!.Borrar(this.entidad!);

            if (resultadoBorrar == null || resultadoBorrar.Id == 0)
                throw new Exception("Error al Borrar ActivosFinancieros a traves de la API.");
